...
class Student(BaseModel):
    ...
    def get_expected_fees(self):
        ...
        fees = AcademicFee.objects.filter(for_level=self.level,stakeholders__contains=[self.departement])
        for fee in fees:
            # we format the payable date(year) to reflet the student's current level(years)
            payable_from = datetime.date(_date.year,fee.payable_from.month,fee.payable_from.day)
            if _date >= payable_from:
                expected_fees.append({
                    'fee':fee,
                    'deadline':payable_from
                })
    ...
    def get_formated_expected_fees(self):
        ...
        for fee in expected_fees:
            fee_category_name = fee.get('fee',None).get_category_display().replace(" ", "")
    ...
    def get_payment_terms(self):
        ...
        for fee in expected_fees:
            ...
            invoices = self.invoices.filter(type=fee.get('fee',None),level=fee.get('fee',None).for_level)
            fee_category_name = fee.get('fee',None).get_category_display().replace(" ", "")
            ...
            if len(invoices)>0:
                completed = invoices.aggregate(Sum('amount'))['amount__sum']==fee.get('fee',None).amount
                paid = True
                ...
                payment_terms[fee_category_name]['total_paid_amount'] = invoices.aggregate(Sum('amount'))['amount__sum']
...
class AcademicFee(BaseModel):
    ...
    stakeholders = ArrayField(models.CharField(max_length=3,choices=Student.DepartementsChoices.choices))
...
class Invoice(BaseModel):
    amount = models.DecimalField(max_digits=16,decimal_places=3)
    level = models.CharField(max_length=4)
    type = models.ForeignKey(AcademicFee,on_delete=models.CASCADE,related_name='payements')
