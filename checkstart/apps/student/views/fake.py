...
from checkstart.apps.student.models import Student,AcademicFee,Invoice
...
class CreateFakeUsersView(APIView):
    def post(self,request):
        ...
        return Response(
            f'{i+1} fake users created successfully'
        )
...
class CreateFakeStudentView(APIView):
    ...
    def post(self,request):
        ...
        return Response(
            'fake students generated successfully'
        )
...
class CreateFakeInvoicesView(APIView):
    ...
    all_stack = ['100','200','300','400','350','450','550','650','500','600','120','220']
    ...
    def post(self,request):
        ...
        for i in range(10):
            ...
            try:
                ac_fee = AcademicFee.objects.get(for_level=level,category=type,stakeholders=stack)
            ...
            student_last_invoice = student.invoices.filter(level=level,type=ac_fee).last()
            
            if student_last_invoice:  
                amount = Decimal(random.randint(1000,int(student_last_invoice.due_amount))) 
